...
if (instance_exists(obj_controller)) {
    if (obj_controller.zoomed = 1) then with(obj_controller) {
        scr_zoom();
    }
}
...
if (image = "fuklaw") and(save > 0) {
    if (press = 1) {
        var del;
        del = obj_saveload.save[self.save];
        if (file_exists("save" + string(del) + ".ini")) {
            file_delete("save" + string(del) + ".ini");
            if (file_exists("save" + string(del) + "log.ini")) {
                file_delete("save" + string(del) + "log.ini");
            }
            if (file_exists("screen" + string(del) + ".png")) {
                file_delete("screen" + string(del) + ".png");
            }
            with(obj_saveload) {
                instance_destroy();
            }
            var news;
            news = instance_create(0, 0, obj_saveload);
    ...
    if (planet = 1) {
        ...
        if (press = 3) {
            ...
            if (instance_exists(flit1)) and(instance_exists(flit2)) {
                if (point_distance(x, y, flit1.x, flit1.y) > point_distance(x, y, flit2.x, flit2.y)) then with(flit2) {
                    instance_destroy();
                }
                else with(flit1) {
                    instance_destroy();
            ...
            if (onceh = 0) and(!instance_exists(flit1)) and(instance_exists(flit2)) {
                if (point_distance(x, y, flit2.x, flit2.y) <= 40) then with(flit2) {
                    instance_destroy();
                }
                onceh = 1;
            }
            if (onceh = 0) and(instance_exists(flit1)) and(!instance_exists(flit2)) {
                if (point_distance(x, y, flit1.x, flit1.y) <= 40) then with(flit1) {
                    instance_destroy();
                }
                onceh = 1;
            }
...
if (image = "inquisition") and(loc = "contraband") {
    ...
    if (press = 1) {
        ...
        repeat(obj_controller.artifacts) {
            ...
            if (string_count("aemon", obj_ini.artifact_tags[e]) > 0) or(string_count("haos", obj_ini.artifact_tags[e]) > 0) {
                ...
                obj_ini.artifact[e] = obj_ini.artifact[e + 1];
                obj_ini.artifact_tags[e] = obj_ini.artifact_tags[e + 1];
                obj_ini.artifact_identified[e] = obj_ini.artifact_identified[e + 1];
                obj_ini.artifact_condition[e] = obj_ini.artifact_condition[e + 1];
                obj_ini.artifact_loc[e] = obj_ini.artifact_loc[e + 1];
                obj_ini.artifact_sid[e] = obj_ini.artifact_sid[e + 1];
            }
        }
        ...
        noom1 = 0;
        noom2 = 0;
        noom1 = instance_nearest(obj_temp_arti.x, obj_temp_arti.y, obj_star);
        noom2 = noom1.name;
        repeat(4400) {
            if (ca <= 10) and(ca >= 0) {
                ia += 1;
    ...
    if (press = 1) {
        ...
        option2 = "";
        loc = "";
        text = "All Chaos and Daemonic Artifacts present have been handed over to the Inquisitor.  They remain seething, but your destruction has been stalled.  Or so you imagine.";
        exit;
    }
    if (press = 2) {
        obj_controller.cooldown = 10;
...
if (image = "mechanicus") and(title = "Mechanicus Mission") or(title = "Mechanicus Mission Accepted") {
    ...
    if (press = 1) and(option1 != "") {
        ...
        if (string_count("tomb", mission) > 0) {
            ...
            with(obj_star) {
                if (name = obj_popup.loc) then instance_create(x, y, obj_temp5);
            }
            if (instance_exists(obj_temp5)) {
                ...
                if (eh > 0) {
                    repeat(4) {
                        eh2 += 1;
                        if (tempy.p_problem[that, eh2] = "") and(that2 = 0) then that2 = eh2;
                    }
                    if (that2 > 0) {
        ...
        if (string_count("raider", mission) > 0) or(string_count("bionics", mission) > 0) or(string_count("mech_mars", mission) > 0) {
            ...
            if (instance_exists(obj_temp5)) {
                ...
                repeat(4) {
                    eh += 1;
                    if (tempy.p_owner[eh] = 3) and(tempy.p_type[eh] = "Forge") then that = eh;
                }
                ...
                if (eh > 0) {
                    ...
                    if (that2 > 0) {
                        ...
                        instance_create(tempy.x + 16, tempy.y - 24, obj_star_event);
                        tempy.p_timer[that, that2] = 49;
                        if (string_count("mars", mission) > 0) then tempy.p_timer[that, that2] = 13;
                        title = "Mechanicus Mission Accepted";
                        option1 = "";
                        option2 = "";
                        cooldown = 15;
...
if ((title = "Inquisition Mission") or(title = "Inquisition Recon")) and(option1 = "") {
    option1 = "Accept";
    option2 = "Refuse";
}
...
if (press = 1) and(option1 != "") or((demand = 1) and(mission != "") and(string_count("Inquisition", title) > 0)) or((demand = 1) and(title = "Inquisition Recon")) {
    if (image = "gene_bad") {
        var onceh;
        onceh = 0;
        if (obj_controller.gene_seed <= 30) and(onceh = 0) {
            obj_controller.gene_seed = 0;
        }
        if (obj_controller.gene_seed > 30) and(obj_controller.gene_seed < 60) and(onceh = 0) {
            obj_controller.gene_seed = round(obj_controller.gene_seed * 0.5);
        }
        if (obj_controller.gene_seed >= 60) and(onceh = 0) {
    ...
    if (title = "Inquisitor Located") or(title = "Artifact Offered") or(title = "Mercy Plea") {
        with(obj_en_fleet) {
            if (trade_goods = "male_her") or(trade_goods = "female_her") then instance_destroy();
        }
    ...
    if (title = "He Built It") {
        obj_ini.race[ma_co, ma_id] = 0;
        obj_ini.loc[ma_co, ma_id] = "";
        obj_ini.name[ma_co, ma_id] = "";
        obj_ini.role[ma_co, ma_id] = "";
        obj_ini.wep1[ma_co, ma_id] = "";
        obj_ini.lid[ma_co, ma_id] = 0;
        obj_ini.wep2[ma_co, ma_id] = "";
        obj_ini.armour[ma_co, ma_id] = "";
        obj_ini.gear[ma_co, ma_id] = "";
        obj_ini.hp[ma_co, ma_id] = 100;
        obj_ini.chaos[ma_co, ma_id] = 0;
        obj_ini.experience[ma_co, ma_id] = 0;
        obj_ini.mobi[ma_co, ma_id] = "";
        obj_ini.age[ma_co, ma_id] = 0;
        with(obj_ini) {
            scr_company_order(0);
        }
    }
    ...
    if (title = "Necron Tomb Excursion") {
        ...
        with(obj_star) {
            if (name != obj_popup.loc) then instance_deactivate_object(id);
        }
    ...
    if (string_count("Necron Tunnels", title) > 0) {
        ...
        if (roll <= 60) {
            ...
            if (obj_temp8.stage >= 4) {
                ...
                instance_activate_object(obj_star);
                with(obj_star) {
                    if (name != obj_controller.temp[200]) then instance_deactivate_object(id);
                }
                ...
                var ppp;
                ppp = 0;
                if (you.p_problem[self.planet, 1] = "bomb") {
                    ppp = 1;
                    you.p_feature[self.planet][search_planet_features(you.p_feature[self.planet], P_features.Necron_Tomb)[0]].sealed = 1;
                    you.p_problem[self.planet, 1] = "";
                    you.p_timer[self.planet, 1] = 0;
                }
                if (you.p_problem[self.planet, 2] = "bomb") {
                    ppp = 2;
                    you.p_feature[self.planet][search_planet_features(you.p_feature[self.planet], P_features.Necron_Tomb)[0]].sealed = 1;
                    you.p_problem[self.planet, 2] = "";
                    you.p_timer[self.planet, 2] = 0;
                }
                if (you.p_problem[self.planet, 3] = "bomb") {
                    ppp = 3;
                    you.p_feature[self.planet][search_planet_features(you.p_feature[self.planet], P_features.Necron_Tomb)[0]].sealed = 1;
                    you.p_problem[self.planet, 3] = "";
                    you.p_timer[self.planet, 3] = 0;
                }
                if (you.p_problem[self.planet, 4] = "bomb") {
                    ppp = 4;
                    you.p_feature[self.planet][search_planet_features(you.p_feature[self.planet], P_features.Necron_Tomb)[0]].sealed = 1;
                    you.p_problem[self.planet, 4] = "";
                    you.p_timer[self.planet, 4] = 0;
                }
                with(obj_temp5) {
                    instance_destroy();
                }
                ...
                var have_bomb;
                have_bomb = scr_check_equip("Plasma Bomb", self.loc, self.planet, 1);
                exit;
            }
        }
    ...
    if (title = "Inquisition Recon") {
        ...
        var pt1, pt2, pt3, pt4, pt5, you, onceh;
        onceh = 0;
        obj_controller.temp[200] = string(loc);

        with(obj_star) {
            if (name = obj_controller.temp[200]) then instance_create(x, y, obj_temp5);
        }
        you = instance_nearest(obj_temp5.x, obj_temp5.y, obj_star);
        onceh = 0;

        if (you.p_problem[planet, 1] = "") and(onceh = 0) {
            you.p_problem[planet, 1] = "recon";
            you.p_timer[planet, 1] = estimate;
            onceh = 1;
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to land Astartes on " + string(you.name) + " " + scr_roman(1) + " to investigate the planet within " + string(estimate) + " months.";
            }
        }
        if (you.p_problem[planet, 2] = "") and(onceh = 0) {
            you.p_problem[planet, 2] = "recon";
            you.p_timer[planet, 2] = estimate;
            onceh = 1;
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to land Astartes on " + string(you.name) + " " + scr_roman(2) + " to investigate the planet within " + string(estimate) + " months.";
            }
        }
        if (you.p_problem[planet, 3] = "") and(onceh = 0) {
            you.p_problem[planet, 3] = "recon";
            you.p_timer[planet, 3] = estimate;
            onceh = 1;
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to land Astartes on " + string(you.name) + " " + scr_roman(3) + " to investigate the planet within " + string(estimate) + " months.";
            }
        }
        if (you.p_problem[planet, 4] = "") and(onceh = 0) {
            you.p_problem[planet, 4] = "recon";
            you.p_timer[planet, 4] = estimate;
            onceh = 1;
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to land Astartes on " + string(you.name) + " " + scr_roman(4) + " to investigate the planet within " + string(estimate) + " months.";
            }
        }
        if (onceh != 0) {
            var bob;
            bob = instance_create(you.x + 16, you.y - 24, obj_star_event);
            bob.image_alpha = 1;
            bob.image_speed = 1;
        }
    ...
    if (mission != "") and(title = "Inquisition Mission") {
        ...
        if (mission = "purge") {
            ...
            repeat(4) {
                s += 1;
                if (you.p_problem[planet, s] = "") and(onceh = 0) {
                    you.p_problem[planet, s] = "purge";
                    you.p_timer[planet, s] = estimate;
                    onceh = s;
                }
            }
            if (onceh != 0) {
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to selectively purge the Nobles on " + string(you.name) + " " + scr_roman(onceh) + " within " + string(estimate) + " months.";
            }
        }
        ...
        if (mission = "cleanse") {
            ...
            repeat(4) {
                s += 1;
                if (you.p_problem[planet, s] = "") and(onceh = 0) {
                    you.p_problem[planet, s] = "cleanse";
                    you.p_timer[planet, s] = estimate;
                    onceh = s;
                }
            }
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to cleanse by fire the mutants in Hive " + string(you.name) + " " + scr_roman(onceh) + " within " + string(estimate) + " months.";
            }
        }

        if (mission = "inquisitor") {
            with(obj_star) {
                if (name = obj_controller.temp[200]) then instance_create(x, y, obj_temp5);
            }
            you = instance_nearest(obj_temp5.x, obj_temp5.y, obj_star);
            ...
            repeat(4) {
                s += 1;
                if (you.p_problem[planet, s] = "") and(onceh = 0) {
                    you.p_problem[planet, s] = "inquisitor" + string(planet);
                    you.p_timer[planet, s] = estimate;
                    onceh = s;
                }
            }
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  A radical inquisitor is enroute to " + string(you.name) + ", expected within " + string(estimate) + " months.  They are to be silenced and removed.";
            }
        }
        ...
        if (mission = "spyrer") {
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  An out of control Spyrer on Hive " + string(you.name) + " " + scr_roman(onceh) + " must be removed within " + string(estimate) + " months.";
            }
        }
        ...
        if (mission = "artifact") {
            ...
            repeat(100) {
                if (last_artifact = 0) {
                    i += 1;
                    if (obj_ini.artifact[i] = "") then last_artifact = i - 1;
                }
            }
        ...
        if (mission = "necron") {
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty.  You have been given a Plasma Bomb to seal the Necron Tomb on " + string(you.name) + " " + scr_roman(onceh) + ".  It is expected to be completed within " + string(estimate) + " months.";
            }
            option1 = "";
            option2 = "";
            option3 = "";
        ...
        if (mission = "tyranid_org") {
            ...
            repeat(4) {
                s += 1;
                if (you.p_problem[planet, s] = "") and(onceh = 0) {
                    you.p_problem[planet, s] = "tyranid_org";
                    you.p_timer[planet, s] = estimate;
                    onceh = s;
                }
            }
            ...
            if (demand = 1) {
                title = "Inquisition Mission Demand";
                text = "The Inquisition demands that your Chapter demonstrate its loyalty to the Imperium of Mankind and the Emperor.  You are to capture a Gaunt organism and return it, unharmed- 4x Webbers have been provided for this purpose.";
            }
        ...
        if (mission = "ethereal") {
            with(obj_star) {
                if (p_tau[1] >= 4) or(p_tau[2] >= 4) or(p_tau[3] >= 4) or(p_tau[4] >= 4) {
                    var bob;
                    bob = instance_create(x + 16, y - 24, obj_star_event);
                    bob.image_alpha = 1;
                    bob.image_speed = 1;
                    bob.color = "green";
                }
...
if (press = 2) and(option2 != "") {
    ...
    if (title = "Inquisitor Located") {
        ...
        if (planet = 1) then gender = "he";
        if (planet = 2) then gender = "she";
        if (planet = 1) then gender2 = "his";
        if (planet = 2) then gender2 = "her";
...
if (press = 3) and(option3 != "") {
    ...
    if (image = "artifact") {
        ...
        if (target_comp != 3) and(target_comp != 4) {
            // Here, have this gift
            var plan;
            plan = instance_nearest(obj_temp4.x, obj_temp4.y, obj_star);
