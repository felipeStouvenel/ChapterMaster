...
with(obj_star) {
    ...
    if (space_hulk = 1) or(craftworld = 1) {
        x -= 20000;
        y -= 20000;
    }
}
var sha;
sha = instance_number(obj_temp6) * 1.3; // former
...
if (instance_number(obj_temp5) > 0) and(imp_ships < sha) {
    ...
    rando2 = choose(1, 2, 2, 3, 3, 3);
    var forge;
    forge = instance_nearest(random(room_width), random(room_height), obj_temp5);

    if (rando <= (12) * instance_number(obj_temp5)) {
        var flit;
        flit = instance_create(forge.x, forge.y, obj_en_fleet);
        flit.owner = 2;
        ...
        if (targeted != 0) and(instance_exists(targeted)) {
            ...
            flit.action_x = targeted.x;
            flit.action_y = targeted.y;
            flit.alarm[4] = 1;
        }
    ...
    if (tco <= 10) and(obj_ini.role[tco, tid] != "") {
        ...
        if (g1 != 0) {
            ...
            if (obj_ini.lid[0, g1] > 0) { // Remove from ship
                var man_size;
                man_size = scr_unit_size(obj_ini.armour[0, g1], obj_ini.role[0, g1], true);
...
if (obj_ini.adv[1] = "Scavengers") or(obj_ini.adv[1] = "Scavengers") or(obj_ini.adv[1] = "Scavengers") or(obj_ini.adv[1] = "Scavengers") {
    ...
    if (lroll1 <= 5) {
        ...
        var tix;
        tix = "A " + string(loot) + " has been gifted to the Chapter.";
...
if (instance_number(obj_temp_inq) > target_navy_number) {
    ...
    var him;
    him = instance_nearest(random(room_width), random(room_height), obj_en_fleet);
    if (him.guardsmen_unloaded = 0) and(him.navy = 1) then with(him) {
        instance_destroy();
    }
    with(obj_en_fleet) {
        if (y < -10000) then y += 20000;
    }
}
if (instance_number(obj_temp_inq) < target_navy_number) {
    with(obj_star) {
        ...
        repeat(4) {
            ...
            if (p_type[o] = "Forge") and(p_owner[o] = 3) and(p_orks[o] + p_tau[o] + p_tyranids[o] + p_chaos[o] + p_traitors[o] + p_necrons[o] = 0) {
                if (present_fleet[7] + present_fleet[8] + present_fleet[9] + present_fleet[10] + present_fleet[13] = 0) {
                    good = 1;
                    if (instance_nearest(x + 24, y - 24, obj_en_fleet).navy = 1) then good = 0;
                }
            }
        }
...
if (instance_exists(obj_temp8)) {
    ...
    nav.home_x = x;
    nav.home_y = y;
    with(instance_nearest(newy.x, newy.y, obj_star)) {
        present_fleet[2] += 1;
    }
...
if (gene_tithe = 0) and(faction_status[2] != "War") { // Time for another tithe
    ...
    var expected, onceh, txt, mech_mad;
    onceh = 0;
    txt = "";
    mech_mad = false;
...
var fuck1;
fuck1 = 0;
fuck1 = scr_role_count(obj_ini.role[100, 15], "");
if (fuck1 = 0) then scr_loyalty("Lack of Apothecary", "+");
fuck1 = 0;
fuck1 = scr_role_count(obj_ini.role[100, 16], "");
if (fuck1 = 0) then scr_loyalty("Upset Machine Spirits", "+");
fuck1 = 0;
fuck1 = scr_role_count(obj_ini.role[100, 14], "");
if (fuck1 = 0) then scr_loyalty("Undevout", "+");
...
if (obj_ini.fleet_type != 1) {
    with(obj_p_fleet) {
        if (capital_number <= 0) then instance_deactivate_object(id);
    }
...
with(obj_fleet) {
    if (owner = 4) then instance_create(x, y, obj_temp6);
}
...
if (inspec = 1) and(faction_status[4] != "War") and(obj_ini.fleet_type = 1) and(instance_number(obj_temp6) = 0) { // Find planet near homeworld to have an inquisitor ship pop from
    ...
    with(obj_star) {
        if (owner = 1) then instance_create(x, y, obj_temp3);
    }
...
if (inspec = 1) and(faction_status[4] != "War") and(obj_ini.fleet_type != 1) and(instance_number(obj_temp6) = 0) { // Find planet near homeworld to have an inquisitor ship pop from
    ...
    if (instance_exists(obj_p_fleet)) {
        with(obj_p_fleet) {
            ...
            if (capital_number > 0) and(action != "") {
                instance_create(action_x, action_y, obj_temp5);
            }
        ...
        with(obj_star) {
            if (owner = 6) then instance_deactivate_object(id);
        }
    ...
    if (event[i] != "") and(event_duration[i] > 0) {
        ...
        if (event_duration[i] = 0) {
            ...
            if (string_count("remove_serf", event[i]) > 0) {
                ...
                with(obj_star) {
                    if (name = obj_controller.temp[1007]) then instance_create(x, y, obj_temp5);
                }
                if (instance_exists(obj_temp5)) {
                    pp = instance_nearest(obj_temp5.x, obj_temp5.y, obj_star);
                    pp.dispo[tc] = -10; // Resets
                    var twix;
                    twix = "Inquisition executes Chapter Serf in control of " + string(tb) + " " + string(tc) + " and installs a new Planetary Governor.";
                    if (pp.p_owner[tc] = 1) then pp.p_owner[tc] = pp.p_first[tc];
                    scr_alert("", "", string(twix), 0, 0);
                    scr_event_log("", string(twix));
                }
                with(obj_temp5) {
                    instance_destroy();
                }
            ...
            if (string_count("new_", event[i]) > 0) {
                ...
                if (instance_number(obj_temp5) > 0) {
                    ...
                    repeat(40) {
                        i += 1;
                        if (obj_ini.ship[i] != "") then ship_names += string(obj_ini.ship[i]);
                    ...
                    if (instance_exists(that2)) {
                        ...
                        var bob;
                        bob = instance_create(that2.x + 16, that2.y - 24, obj_star_event);
                        bob.image_alpha = 1;
                        bob.image_speed = 1;
                    }
                }
            ...
            if (string_count("strange_building", event[i]) > 0) {
                ...
                var killy, tixt;
                killy = 0;
                tixt = string(obj_ini.role[100, 16]) + " " + string(bb) + " has finished his work- ";
                ...
                if (ee = "fusion") {
                    ...
                    obj_ini.lid[cc, dd] = 0;
                    obj_ini.wep2[cc, dd] = "";
                    obj_ini.armour[cc, dd] = "";
                    obj_ini.gear[cc, dd] = "";
                    obj_ini.hp[cc, dd] = 100;
                    obj_ini.chaos[cc, dd] = 0;
                    obj_ini.experience[cc, dd] = 0;
                    obj_ini.mobi[cc, dd] = "";
                    obj_ini.age[cc, dd] = 0;
                    with(obj_ini) {
                        scr_company_order(0);
...
if (fest_scheduled > 0) and(fest_repeats > 0) {
    ...
    if (cm_present = true) {
        ...
        else if (fest_type = "Tournament") then imag = "event_tournament";
        else if (fest_type = "Deathmatch") then imag = "event_deathmatch";
        else if (fest_type = "Imperial Mass") then imag = "event_mass";
        else if (fest_type = "Cult Sermon") then imag = "event_ccult";
        else if (fest_type = "Chapter Relic") then imag = "event_ccrelic";
        else if (fest_type = "Triumphal March") then imag = "event_march";
