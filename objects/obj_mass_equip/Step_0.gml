...
if (refresh = true) and(obj_controller.settings > 0) {
    ...
    for (var i = 1; i <= 60; i++) {
        if (we1[i] != "") then wep1_equip += string(we1_n[i]) + "x " + string(we1[i]) + ", ";
        if (we2[i] != "") then wep2_equip += string(we2_n[i]) + "x " + string(we2[i]) + ", ";
        if (arm[i] != "") then armour_equip += string(arm_n[i]) + "x " + string(arm[i]) + ", ";
        if (gea[i] != "") then gear_equip += string(gea_n[i]) + "x " + string(gea[i]) + ", ";
        if (mob[i] != "") then mobi_equip += string(mob_n[i]) + "x " + string(mob[i]) + ", ";
    }
